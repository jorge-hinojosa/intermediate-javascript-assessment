...
function accountingOffice(assets) {
  return function acc(liabilities) {
...
function frodo(startingHungerValue, startingDangerValue) {
  ...
  return {
    dinnerOverFire: () => {
      return {
        hunger:
          (startingHungerValue -= 25) < minimum
            ? 0
            : (startingHungerValue -= 25),
        danger:
          (startingDangerValue += 40) > maximum
            ? 100
    ...
    hidingInBush: () => {
      return {
        hunger:
          (startingHungerValue += 35) > maximum
            ? 100
            : (startingHungerValue += 35),
        danger:
          (startingDangerValue -= 20) < minimum
            ? 0
